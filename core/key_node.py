...
Identifies diagnostically significant nodes using 5-dimension scoring:
...
3. Temporal Correlation (时序相关性) - Freshness + recency score
4. Clinical Significance (临床意义) - Node weight from entity graph
5. Community Role (社区角色) - Bridge/hub detection in relation graph
"""
...
class KeyNodeIdentifier:
    """
    Identifies key diagnostic nodes using 5-dimension scoring.
    ...
    def __init__(self, entity_graph: nx.DiGraph, relation_graph: nx.DiGraph, config: ConfigManager):
        ...
        Args:
            entity_graph: Entity graph with node attributes (weight, temporal_confidence, etc.)
        ...
        self.temporal_weight = getattr(config.system, 'temporal_weight', self.DEFAULT_TEMPORAL_WEIGHT)
        ...
        total = (self.centrality_weight + self.confidence_weight + self.temporal_weight + 
    ...
    def identify(
        ...
        """
        Identify key nodes based on 5-dimension scoring.
        ...
        temporal_scores = self._compute_temporal_correlation_scores()
        ...
        for node_id in node_ids:
            ...
            temporal = temporal_scores.get(node_id, 0.0)
    ...
    def _compute_confidence_scores(self) -> Dict[str, float]:
        """
        Extract confidence scores from entity graph temporal_confidence attribute.
        ...
        for node_id in self.entity_graph.nodes():
            node_data = self.entity_graph.nodes[node_id]
            # Use temporal_confidence (already computed by temporal decay module)
            confidence = node_data.get("temporal_confidence", 
                                      node_data.get("confidential_level", 0.5))
            confidence_scores[node_id] = float(confidence)
        
        return confidence_scores
    
    def _compute_temporal_correlation_scores(self) -> Dict[str, float]:
        """
        Compute temporal correlation scores from freshness and recency.
        
        Temporal correlation measures how recently and frequently the node was updated.
        High score = information is fresh and stable.
        
        Returns:
            Dictionary mapping node_id -> temporal correlation score
        """
        temporal_scores = {}
        now = datetime.now()
        ...
        for node_id in self.entity_graph.nodes():
            ...
            # Get freshness (already computed by temporal decay module)
            freshness = node_data.get("freshness", 0.5)
            
            # Compute recency score based on last_updated_at
            last_updated = node_data.get("last_updated_at")
            if last_updated:
            ...
            # Combine freshness and recency (60% freshness, 40% recency)
            temporal_scores[node_id] = 0.6 * freshness + 0.4 * recency
    ...
    def _compute_combined_scores(self) -> Dict[str, float]:
        ...
        temporal = self._compute_temporal_correlation_scores()
        ...
        for node_id in self.entity_graph.nodes():
            score = (
                ...
                self.temporal_weight * temporal.get(node_id, 0.0) +
